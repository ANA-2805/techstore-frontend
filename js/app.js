...
async function login() {
  ...
  } catch(e) { showAlert('loginError', e.message || 'Credenciales incorrectas.'); }
}
async function register() {
  ...
  } catch(e) { showAlert('registerError', e.message); }
}
function logout() { usuarioActual = null; carrito = []; showScreen('loginScreen'); }

async function cargarDashboard() {
  try {
    const [prods, vents] = await Promise.all([apiProductos.getAll(), apiVentas.getAll()]);
    productos = prods; ventas = vents;
    ...
    try { const r = await apiVentas.totalVendido(); totalAcumulado = r.total || totalAcumulado; } catch(_) {}
    ...
    stockBajo.slice(0,8).forEach(p => {
      tbodyS.innerHTML += `<tr><td>${p.nombre}</td><td><span class="badge ${p.stock===0?'badge-bad':'badge-warn'}">${p.stock}</span></td></tr>`;
    });
  } catch(e) { console.error(e); }
}

async function cargarProductos() {
  try { productos = await apiProductos.getAll(); renderProductos(productos); }
...
function renderProductos(lista) {
  ...
  lista.forEach(p => {
    ...
    tbody.innerHTML += `<tr>
      <td>${p.nombre}</td><td>${p.categoria||'—'}</td><td>${fmt(p.precio)}</td>
...
function openModalProducto() {
  ...
  ['productoNombre','productoCategoria','productoPrecio','productoStock','productoDescripcion'].forEach(id => document.getElementById(id).value='');
...
async function guardarProducto() {
  ...
  try {
    ...
    closeModal('modalProducto'); cargarProductos();
  } catch(e) { showToast(e.message,'error'); }
}
async function eliminarProducto(id) {
  if (!confirm('¿Eliminar este producto?')) return;
  try { await apiProductos.delete(id); showToast('Producto eliminado.'); cargarProductos(); }
  catch(e) { showToast(e.message,'error'); }
}

async function cargarClientes() {
  try { clientes = await apiClientes.getAll(); renderClientes(clientes); poblarSelectCliente(); }
...
function renderClientes(lista) {
  ...
  tbody.innerHTML = '';
  lista.forEach(c => {
    tbody.innerHTML += `<tr><td>${c.nombre}</td><td>${c.email}</td><td>${c.telefono||'—'}</td>
      <td><button class="btn btn-secondary btn-sm" onclick="editarCliente('${c._id}')">Editar</button></td></tr>`;
  });
}
...
async function guardarCliente() {
  ...
  try {
    ...
    closeModal('modalCliente'); cargarClientes();
  } catch(e) { showToast(e.message,'error'); }
}

async function cargarVentas() {
  try {
    [productos,clientes,ventas] = await Promise.all([apiProductos.getAll(),apiClientes.getAll(),apiVentas.getAll()]);
    poblarSelectProducto(); poblarSelectCliente(); renderVentas();
  } catch(e) { showToast('Error al cargar ventas.','error'); }
}
function poblarSelectProducto() {
  const sel = document.getElementById('ventaProductoSelect'); if(!sel) return;
  sel.innerHTML = '<option value="">Seleccionar producto...</option>';
  productos.forEach(p => { sel.innerHTML += `<option value="${p._id}" data-precio="${p.precio}" data-stock="${p.stock}">${p.nombre} — ${fmt(p.precio)}</option>`; });
}
function poblarSelectCliente() {
  const sel = document.getElementById('ventaCliente'); if(!sel) return;
  sel.innerHTML = '<option value="">Seleccionar cliente...</option>';
  clientes.forEach(c => { sel.innerHTML += `<option value="${c._id}">${c.nombre}</option>`; });
}
function agregarItemVenta() {
  ...
  if (exist) { exist.cantidad+=cant; } else { carrito.push({id:opt.value,nombre:opt.text.split(' — ')[0],precio,cantidad:cant}); }
  renderCarrito();
}
function quitarItemCarrito(id) { carrito=carrito.filter(i => i.id!==id); renderCarrito(); }
function renderCarrito() {
  const tbody = document.querySelector('#tablaCarrito tbody');
  tbody.innerHTML = ''; let total = 0;
  carrito.forEach(i => {
    const sub = i.precio*i.cantidad; total+=sub;
    tbody.innerHTML += `<tr><td>${i.nombre}</td><td>${fmt(i.precio)}</td><td>${i.cantidad}</td><td>${fmt(sub)}</td>
      <td><button class="btn btn-danger btn-sm" onclick="quitarItemCarrito('${i.id}')">✕</button></td></tr>`;
  });
...
async function registrarVenta() {
  ...
  if (!carrito.length) return showToast('Agrega al menos un producto.','error');
  const total = carrito.reduce((s,i) => s+i.precio*i.cantidad, 0);
  const cliente = clientes.find(c => c._id===clienteId);
  const data = { clienteId, clienteNombre:cliente?.nombre||'', productos:carrito.map(i => ({productoId:i.id,nombre:i.nombre,precio:i.precio,cantidad:i.cantidad})), total, fecha:new Date().toISOString() };
  try {
    const venta = await apiVentas.create(data);
    showToast('Venta registrada correctamente.');
    generarTicket(venta,cliente); carrito=[]; renderCarrito(); cargarVentas();
  } catch(e) { showToast(e.message,'error'); }
}
function renderVentas() {
  ...
  [...ventas].reverse().slice(0,20).forEach(v => {
    tbody.innerHTML += `<tr><td>${fmtFecha(v.fecha)}</td><td>${v.clienteNombre||'—'}</td><td>${fmt(v.total)}</td>
      <td><button class="btn btn-secondary btn-sm" onclick="verTicketVenta('${v._id}')">Ver</button></td></tr>`;
  });
}
function verTicketVenta(id) { const v=ventas.find(x => x._id===id); if(!v) return; generarTicket(v,{nombre:v.clienteNombre}); }
function generarTicket(venta,cliente) {
  const lineas=(venta.productos||[]).map(p => `<div class="ticket-row"><span>${p.nombre} x${p.cantidad}</span><span>${fmt(p.precio*p.cantidad)}</span></div>`).join('');
  document.getElementById('ticketContent').innerHTML=`
    <div class="ticket-header"><strong>⬡ TechStore</strong><br/>${fmtFecha(venta.fecha||new Date())}</div>
    <hr class="ticket-divider"/>
    <div class="ticket-row"><span>Cliente:</span><span>${cliente?.nombre||'—'}</span></div>
    <hr class="ticket-divider"/>${lineas}<hr class="ticket-divider"/>
...
function imprimirTicket() {
  ...
  win.document.write(`<html><head><title>Ticket TechStore</title><style>body{font-family:'Courier New',monospace;font-size:12px;padding:1rem}.ticket-row{display:flex;justify-content:space-between}hr{border:none;border-top:1px dashed #aaa;margin:.5rem 0}</style></head><body>${document.getElementById('ticketContent').innerHTML}</body></html>`);
  win.print(); win.close();
}
...
async function reporteVentasFecha() {
  ...
  try {
    ...
    const tbody=document.querySelector('#tablaReporteVentas tbody'); tbody.innerHTML='';
    data.forEach(v => { tbody.innerHTML+=`<tr><td>${fmtFecha(v.fecha)}</td><td>${v.clienteNombre||'—'}</td><td>${fmt(v.total)}</td></tr>`; });
...
async function reporteProductosMasVendidos() {
  try {
    ...
    const tbody=document.querySelector('#tablaReporteProductos tbody'); tbody.innerHTML='';
    data.forEach(p => { tbody.innerHTML+=`<tr><td>${p.nombre}</td><td>${p.totalVendido}</td></tr>`; });
  } catch(e) { showToast(e.message,'error'); }
}
async function reporteInventario() {
  try {
    ...
    data.forEach(p => {
      ...
      tbody.innerHTML+=`<tr><td>${p.nombre}</td><td>${p.stock}</td><td><span class="badge ${estado[0]}">${estado[1]}</span></td></tr>`;
    });
  } catch(e) { showToast(e.message,'error'); }
}
async function reporteVentasAltas() {
  try {
    ...
    const tbody=document.querySelector('#tablaVentasAltas tbody'); tbody.innerHTML='';
    data.forEach(v => { tbody.innerHTML+=`<tr><td>${fmtFecha(v.fecha)}</td><td>${v.clienteNombre||'—'}</td><td>${fmt(v.total)}</td></tr>`; });
